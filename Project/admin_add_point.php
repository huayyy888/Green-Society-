...
<body>
    ...
    $admin_id = $_SESSION['nokp'];
    $sql_admin = "SELECT * FROM admin WHERE admin_nokp = $admin_id";
    $result_admin = mysqli_query($combine_data, $sql_admin);

    if($result_admin && mysqli_num_rows($result_admin) > 0) {
        $row = mysqli_fetch_assoc($result_admin);
        $admin_id = $row['admin_nokp'];
    ...
    <button class="button back-button hide-print" onclick="location.href='admin_user_manage.php'" title="Back to Event List"><i class="fa-solid fa-circle-left"></i> Back</button>
        ...
        <button class="button hide-item" onclick="if(confirm('Click OK to delete all table data.')) { location.href='E003_delete_all_data.php'; }" title="Click to Delete All Table Data!" class='hide-item'><i class="fa-solid fa-trash-can"></i> Delete All Table Data</button>
        
        <div class="hide-print">
            <br>
        </div>

        <form action="admin_add_point_process.php?total=<?php echo $all_total; ?>&member_id=<?php echo $login_id; ?>" method="POST" onsubmit="return confirm('Click OK to submit the data.');" enctype="multipart/form-data">
        ...
        <button type="button" class="button hide-item print_button" onclick="printScreen(<?php echo $table_no; ?>)" title="Print Screen">Print Screen</button>
            ...
            <div class="hide-item upload_pdf">
                <label for="file">Upload Your Print Screen Here in PDF file:</label><br>
                <input id="file" type="file" name="file" accept=".pdf" required/>
            </div>

            <br>

            <button type="submit" class="button hide-item" title="Click to Submit The Data!"><i class="fa-solid fa-check"></i> Click to Submit The Data</button>
